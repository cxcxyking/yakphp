...
class ApplicationActionNotFoundHandler extends Handler
{
	...
	private static function do(YakRouteIntent $origin)
	{
		Hook::register('Yak.Launcher.launchApplication.after', function ($id) use ($origin) {
			...
			Launcher::launchApplicationWithIntent(
				...
				new YakRouteIntent($origin->getApplication(), ['action' => [\YakInstance\ComponentNotFound\Controller\Home::class, 'actionNotFound']], ['target' => $origin->getTarget()])
			);
		});
	}
}